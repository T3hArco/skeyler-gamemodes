...
function GM:ScoreboardShow()
	...
	RenderDistanceSlider = vgui.Create("DSlider", DermaFrame)
	local distw, disth = surface.GetTextSize("Render Distance: AAAAAA")
	RenderDistanceSlider:SetPos((DermaFrame:GetWide()/3)*2 + (distw)/2, tabheight - (tabHeightBuffer/4)*3 )
	RenderDistanceSlider:SetSlideX((renderConvar:GetFloat()/2560))	
	RenderDistanceSlider.Paint = function()
		LocalPlayer():ConCommand("sass_buildingdistance " .. RenderDistanceSlider.m_fSlideX*2560)
		surface.DrawLine(0, RenderDistanceSlider:GetTall()/2, RenderDistanceSlider:GetWide(), RenderDistanceSlider:GetTall()/2)
	end
	...
	function DermaFrame:Paint()
		...
		local distw, disth = surface.GetTextSize("Render Distance: ")
		surface.SetTextPos( (DermaFrame:GetWide()/3)*2 + (distw)/2, tabheight - (tabHeightBuffer/4)*3) 
		surface.DrawText( math.Round(renderConvar:GetFloat()) )
